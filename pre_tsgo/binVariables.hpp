...
/*
	This function takes a graphical model with a bunch of unaries.
	It then goes on to bin these unaries (and hence their corresponding variables)
		into equally space partitions per dimension.
*/
template<class T, class OPERATOR, class FUNCTION_TYPE_LIST, class SPACE>
size_t equallyDividedBins(
	// arguments
		const opengm::GraphicalModel<T, OPERATOR, FUNCTION_TYPE_LIST, SPACE>& gm, 
		int nBinsPerUnaryDimension, // this argument should get more special as time goes on.
		std::vector<int>& colors
	) { // function begins TODO
	int nVar = gm.numberOfVariables();
	std::vector< int > varId2UnaryIds(nVar, -1 );
	int nLabels = gm.numberOfLabels(0);
	std::vector<double> minPerLabel(nLabels, -1.0);
	std::vector<double> maxPerLabel(nLabels, -1.0);
	

	// get minperlabel and maxperlabel
	int nFactorsOfVid = 0;
	for(int vid=0; vid<nVar; ++vid) {
		// get the unary potential function.
		nFactorsOfVid = gm.numberOfFactors(vid);
		
		for(int fidid=0; fidid<nFactorsOfVid; ++fidid) {
			// if fidid is unary, then figure out the dimension thingy.
			int fid = gm.factorOfVariable(vid, fidid);
			if (gm[fid].dimension()==1) { // unary
				// remember the unary.
				varId2UnaryIds[vid] = (fid);
				
				// update min max.
				size_t iterator[] = {0};
				for(size_t l=0; l<nLabels; ++l) {
					iterator[0] = l;
					double val = gm[fid].template operator()<size_t*>(iterator);
					if (val>maxPerLabel[l] || maxPerLabel[l]==-1.0) { maxPerLabel[l] = val; }
					if (val<minPerLabel[l] || minPerLabel[l]==-1.0) { minPerLabel[l] = val;	}
				}
				break; // only one unary.
			}
		}
	}

	// ASSERT: we have the min & max values per bin.

	size_t nColorsUsed = 0;
	// give each variable's unary potential a color.
	int fid;
	std::map< std::vector<int> , int > discreteUnary2Color;
	for(int vid=0; vid<nVar; ++vid) {
		// get the unary potential function.
		fid = varId2UnaryIds[vid];
		// get the discretized version of the unary.
		std::vector<int> discreteUnary(nLabels);
		double val;
		size_t iter[] = {0};
		for(size_t lid=0; lid<nLabels; ++lid) {
			iter[0] = lid;
			val = gm[fid].template operator()<size_t*>(iter);
			double binSizeOfLid = double((maxPerLabel[lid] - minPerLabel[lid])/nBinsPerUnaryDimension);
			discreteUnary[lid] = int((val - minPerLabel[lid])/binSizeOfLid);
		}
		if (discreteUnary2Color.count(discreteUnary) != 0) {
			// nothing to do... its being done outside the if-else block.
		} else {
			discreteUnary2Color[discreteUnary] = nColorsUsed;
			nColorsUsed++;
		}
		int color = discreteUnary2Color[discreteUnary];
		colors[fid] = color;
	}
...
size_t initializeFactorColors(
	...
	for(int fid=0; fid<facColor.size(); ++fid) {
		...
		// }
		std::cout << "iteration=" << fid << "/" << facColor.size() << "\r" << std::flush;
...
template<class T, class OPERATOR, class FUNCTION_TYPE_LIST, class SPACE>
std::vector< std::vector<size_t> > getVarGroupingsUsingBins(
		// arguments
			const opengm::GraphicalModel<T, OPERATOR, FUNCTION_TYPE_LIST, SPACE>& gm, 
	...
	size_t nFacColors = equallyDividedBins<T, OPERATOR, FUNCTION_TYPE_LIST, SPACE>(gm, nBinsPerUnaryDimension ,facColor);
