/**
	...
	it runs stereo matching after using counting bp to detect symmetries in the graph.
...
#define N_STEPS 100
#define N_MAX_ITERATIONS 4
#define L1_EPSILON 1000.0
...
int nMaxIterations=N_MAX_ITERATIONS;
double l1Epsilon = L1_EPSILON;
string symsfile="";
...
void singleInstance(string leftFile, string rightFile, string outputFile, string groundTruth) {
	...
	// reduce the gm using symmetries.
	std::vector< std::vector<size_t> > syms = getVarGroupingsUsingMapSyms
												<double, opengm::Adder, OPENGM_TYPELIST_2(opengm::ExplicitFunction<double> , opengm::TruncatedAbsoluteDifferenceFunction<double> /*opengm::PottsFunction<double>*/ ) , Space>
													(gm, nMaxIterations, nRanksConsidered);
	
	// TODO need to store syms into a photo like thing.
	size_t nGroups = syms.size();
	std::vector<size_t> invsysms(gm.numberOfVariables(), 0);
	...
	cout << "saving symmetries too" << endl;
	...
	if (argc>1) { // need to do better, but ah well.
		if (strcmp("single",argv[1]) == 0) { // singleInstance
			...
			if (argc>5) {
				...
				l1Epsilon=nLabels*atof(argv[10]);
				golden = argv[11];
				symsfile=argv[12];
		...
		cout << "\nUSAGE" << endl;
		cout << "singleInstance: <./a.out> single <path-to-left> <path-to-right> <output-file=output.png> <nlabels=10 lambda=1000 unaryPotentialRadius=3 nStepsOfGC nMaxIterationsOfCountingBP=4 L1Epsilon=1000.0 (later multiplied by nLabels)>" << endl;
		cout << "multipleInstances: <./a.out> multi <path to file contaning list of instances> <nlabels=10 lambda=1000 unaryPotentialRadius=3 nStepsOfGC nMaxIterationsOfCountingBP=4 L1Epsilon=1000.0 (later multiplied by nLabels)>" << endl << endl;
		return 1;
	}
